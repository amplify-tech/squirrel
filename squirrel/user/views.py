...
def myfrnd(request,  user_id=None) :
    if int(user_id) != request.user.id:
        ...
        if request.user.is_authenticated:
            ...
            return render(request, 'myfrnd.html', { 'allfollowing': profile_flwing, 'allfolower': profile_flwer, 'all_notify':total_notify, 'status':allstatus})
        else:
            messages.info(request, 'Please Login First')
            return render(request, 'login.html')
...
def home_likes(request, user_id=None):
    status_id = request.POST['status_id']
    ...
    return redirect('/user/' +str(request.user.id) + '/myfrnd#' + str(status_id))
